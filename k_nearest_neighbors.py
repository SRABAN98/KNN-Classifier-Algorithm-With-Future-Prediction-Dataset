...
plt.title("KNNClassifier(Training set)")
plt.xlabel("Age")
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(x_set[y_set == j, 0], x_set[y_set == j, 1],
                c = ListedColormap(("red", "green"))(i), label = j)
plt.title("KNNClassifier(Test set)")
plt.xlabel("Age")
plt.ylabel("Estimated Salary")
plt.legend()
plt.show()
...
y_pred_KNNClassifier = pd.DataFrame()

d2["y_pred_KNNClassifier"] = classifier.predict(M)


#save the future prediction dataframe as the .csv file format
d2.to_csv("FPofKNNClassifierAlgo.csv")
